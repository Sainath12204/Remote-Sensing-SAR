  ...
  Future<void> _pickImage() async {
    ...
    if (pickedFile != null) {
      setState(() {
        ...
        _detectionResult = ''; // Reset the result text
      });
    }
  }
  ...
  Future<void> _detectFlood() async {
    ...
    try {
      ...
      setState(() {
        ...
        _detectionResult = result['flood_detected']
            ? 'Flood Detected'
            : 'No Flood Detected'; // Display detection result
      });
    } catch (e) {
      setState(() {
        _detectionResult = 'Error detecting flood: $e'; // Handle error
      });
    }
  }
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                _buildImageContainer(_inputImage,
                    'Upload Image'), // Reusable container for input image
                ...
                FilledButton.icon(
                  ...
                  icon:
                      Icon(Icons.flood, size: 30), // Increase icon size
                  label: const Text(
                    "Detect Flood",
                    style: TextStyle(fontSize: 16), // Increase text size
                  ),
                ...
                // Predicted Mask Image Container
                _buildImageContainer(_predictedMaskImage,
                    'Predicted Mask'), // Reusable container for predicted mask image
                const SizedBox(height: 20),

                // Result Image Container
                _buildImageContainer(_resultImage,
                    'Result Image'), // Reusable container for result image
              ],
            ),
          ),
        ));
